...
#initalize players and stack size in player class.
def makeplayer(num,stack,smallblind,bigblind):
    players = []
    for i in xrange(0,number):
        print 'i',i
        subtractblind = 0
        if i == 0:
            subtractblind = smallblind
        if i == 1:
            subtractblind = bigblind
        players.append(Player('',(stack-subtractblind),'U',str(i),subtractblind, False))
        return players

#initialize and deal random hands
...
#decides who won if anyone
def outcome(results,outcome,players):

#main function - inits players, inits reduced deck to use in sims
def main(deck,numplayers,stack,smallblind,bigblind):
    #setting position 0 = btn, 1 = bb
    newdeck = deck[:]
    #player wins,ties, total games.
    results = [[0,0]] * numplayers + [0]
    #player winnings.
    winnings = [0] * numplayers
    #Pot - bettotal = winnings on that street
    #pot + stack - initial stack = total winnings
    for i in xrange(0,1):
        outcome, action, pot = minigame(numplayers,newdeck,stack,smallblind,bigblind)
        print "outcome", outcome
        results[3] += 1
        if outcome == "Tie!":
            results[2] += 1
...
#minigame function
def minigame(numplayers,deck,stack,smallblind,bigblind):
    pot = bigblind + smallblind
    players = makeplayer(numplayers,stack,smallblind,bigblind)
    (players, deck) = makehand(players,deck)
    street = 0
    ...
    newdeck = deck[:]
    if street == 0:
        (outcome, action, board, pot, newdeck, street) = preflop(players,street,newdeck,pot,board,action,smallblind,bigblind)
        if action[-1][0] == 'F':
    ...
    while street < 4:
        ...
        (outcome, action, board, pot, newdeck, street) = postflop(players,street,newdeck,pot,board,action,smallblind,bigblind)
        if action[-1][0] == 'F':
            break
        street = street + 1
        print "action", action
...
#actions start at 2 for preflop and start at 0 for postflop
...
#deck,numplayers,stacksize,smallblind,bigblind
print main(deck,3,100,1,2)
