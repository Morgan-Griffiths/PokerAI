...
class PreProcessLayer(nn.Module):
    def __init__(self,params):
        ...
        self.action_emb = nn.Embedding(embedding_dim=self.emb, num_embeddings=6)
        self.position_emb = nn.Embedding(embedding_dim=self.emb, num_embeddings=3)
        self.hand_fc = nn.Linear(1024,self.emb)
        self.position_fc = nn.Linear(self.emb,self.emb)
        ...
        # self.hand_transformer = CTransformer(emb=1024,heads=16,depth=4,seq_length=self.maxlen,num_classes=params['embedding_size'])
        # self.action_lstm = nn.LSTM(params['embedding_size'],params['embedding_size'])
        # self.position_lstm = nn.LSTM(params['embedding_size'],params['embedding_size'])
        # self.betsize_lstm = nn.LSTM(1,params['embedding_size'])
        # self.hand_lstm = nn.LSTM(1024,params['embedding_size'])
    ...
    def forward(self,x):
        ...
        last_p = x[:,:,self.state_mapping['hero_position']].long()
        # emb_a = self.action_emb(last_a)
        emb_p = self.position_emb(last_p)
        hand_out = self.hand_fc(h)
        ...
        combined = torch.cat((hand_out,emb_p),dim=-1)
        # print(combined.size())
        return combined
