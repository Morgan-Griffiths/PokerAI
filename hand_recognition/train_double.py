...
def train_network(id,data_dict,agent_params,training_params):
    ...
    if training_params['frozen']:
        ...
        fc_path = '../poker/checkpoints/frozen_layers/hand_board_weights'
    ...
    for epoch in range(training_params['epochs']):
        ...
        for i, data in enumerate(data_dict['trainloader'], 1):
            ...
            optimizer.zero_grad()
            hero_dist,villain_dist,perceived_dist = net(inputs)
            hero_loss = criterion(hero_dist, targets[:,0]).sum()
            villain_loss = criterion(villain_dist, targets[:,1]).sum()
            perceived_loss = criterion(perceived_dist, targets[:,1]).sum()
            loss = hero_loss + villain_loss + perceived_loss
            loss.backward()
            optimizer.step()
            ...
            sys.stdout.flush()
        if id == 0 or id == 'cpu':
            print(f'\nMaximum value {torch.max(torch.softmax(hero_dist,dim=-1),dim=-1)[0][:15]}, \nLocation {torch.argmax(hero_dist,dim=-1)[:15]}')
            print('targets',targets[:15,0])
            print(f'Villain max values{torch.max(torch.softmax(villain_dist,dim=-1),dim=-1)[0][:15]}')
            print('\nvillain guesses',torch.argmax(villain_dist,dim=-1)[:15])
            print(f'perceived max values{torch.max(torch.softmax(perceived_dist,dim=-1),dim=-1)[0][:15]}')
            print('\perceived guesses',torch.argmax(perceived_dist,dim=-1)[:15])
            print('villain targets',targets[:15,1])
        lr_stepper.step()
        ...
        for i, data in enumerate(data_dict['valloader'], 1):
            ...
            villain_loss = criterion(villain_dist, targets[:,1]).sum()
            perceived_loss = criterion(perceived_dist, targets[:,1]).sum()
            val_loss = hero_loss + villain_loss + perceived_loss
        ...
        net.train()
        if id == 0 or id == 'cpu':
            # print('\nhero guesses',torch.argmax(hero_dist,dim=-1)[:15])
            # print('hero targets',targets[:15,0])
            # print('\nvillain guesses',torch.argmax(villain_dist,dim=-1)[:15])
            # print('\perceived guesses',torch.argmax(perceived_dist,dim=-1)[:15])
            # print('villain targets',targets[:15,1])
